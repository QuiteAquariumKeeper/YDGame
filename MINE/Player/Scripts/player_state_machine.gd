...
func Initialize( _player : Player ) -> void: # for setting up state machine
	...
	for c in get_children(): # get_children() returns an array of nodes (children of the node this script attached to which is StateMachine), ie for each node we give it a label c
		if c is State:
